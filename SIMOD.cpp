...
private:
    // Perlin noise function for procedural generation
    double perlinNoise(double x, double y, double z) {
        // Implementation of Perlin noise (or use a library if allowed)
        // This is a placeholder for a real Perlin noise algorithm
        return (std::sin(x) + std::cos(y) + std::sin(z)) / 3.0;
    }
};
...
private:
    ...
    cv::Mat simulateEnvironment(const cv::Mat& photo, double time, const std::string& mode) {
        ...
        if (mode == "2D") {
            // Apply simple transformations for 2D navigation
            cv::Mat transform;
            cv::warpAffine(photo, environment, transform, photo.size());
        } else if (mode == "3D") {
            // Simulate depth and perspective for 3D navigation
            cv::Mat depthMap;
            cv::GaussianBlur(photo, depthMap, cv::Size(15, 15), 0);
            environment = depthMap;
        } else if (mode == "4D") {
            // Add dynamic lighting or object movement for 4D effects
            double intensity = std::sin(time) * 50;
            environment += cv::Scalar(intensity, intensity, intensity);
        }
...
private:
    // Extract audio duration (placeholder for real implementation)
    double getAudioDuration(const std::string& audioPath) {
        // Use a library like FFmpeg to extract the duration of the audio file
        return 180.0; // Placeholder: 3 minutes
    }
...
private:
    ...
    double getAudioDuration(const std::string& audioPath) {
        // Placeholder: Use a library like FFmpeg to extract the duration of the audio file
        return 180.0; // Assume 3 minutes for now
    }
    ...
    void attachAudioToVideo(const std::string& audioPath, const std::string& videoPath) {
        std::string command = "ffmpeg -y -i " + videoPath + " -i " + audioPath + " -c:v copy -c:a aac -strict experimental output_with_audio.avi";
